...
public class SortPanel extends JPanel implements AlgorithmDelegate
{
    ...
    public void visualizeData(int [] array)
    {
        ...
        if ((now.getTime()-lastUpdate.getTime())>17)//17 ms = ~1/60 second
        {
            getCanvas();
            if (myCanvas == null)
                return;
            synchronized (myCanvas) //prevent any other method from using the canvas until I am
                                    //     done drawing in it. It will be locked for the duration
            ...
            // since you just updated the canvas, let the panel know that it is ready for an
            //   update. Note: we do this after releasing the "myCanvas" variable, since the
            //   process of repainting will depend on using that variable!
            repaint();
            // reset the 1/60th second clock
            lastUpdate = new Date();
        }
        // pause the program briefly, based on the delay slider.
        try
        {
